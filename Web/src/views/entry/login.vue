  ...
  <div class="container">
    <el-card class="box-card">
      <el-form class="loginFrom" :model="loginFrom" status-icon :rules="loginFromRule" ref="loginFrom"
               @keypress.native.enter="submitForm('loginFrom')">
        <div class="center">
          <div style="font-weight: bold;text-align: center;font-size: 1.5em">登录NTM社团管理系统</div>
        </div>
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$.ajax.post(`/login`, JSON.stringify({
              ...
              this.$router.push(res.role === 'ADMIN' ? '/admin' : '/');
            }, (err) => {
